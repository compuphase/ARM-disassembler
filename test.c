...
int main(int argc, char *argv[])
{
  ...
  if (argc > 1) {
    ...
    if (fp) {
      ...
      unsigned char *block = malloc(fsize);
      if (block) {
        fseek(fp, 0, SEEK_SET);
        ...
        if (argc > 5) {
          ...
          if (ro_addr + ro_size > fsize)
            ro_size = fsize - ro_addr;
          disasm_literals(&arm, block + ro_addr, ro_size, ro_addr);
        }
        size_t offset = (argc > 2) ? strtoul(argv[2], NULL, 0) : 0;
        size_t blocksize = (argc > 3) ? strtoul(argv[3], NULL, 0) : fsize;
        if (offset + blocksize > fsize)
          blocksize = fsize - offset;
        disasm_address(&arm, offset);
        disasm_buffer(&arm, block + offset, blocksize, ARMMODE_THUMB, disasm_callback, NULL);
        disasm_cleanup(&arm);
        free(block);
      } else {
        fprintf(stderr, "Memory allocation error, when attempting to allocate %d bytes.\n", fsize);
      }
      fclose(fp);
    } else {
...
#       define PATH_SEPARATOR '/'
#     endif
      const char *pgmname = strrchr(argv[0], PATH_SEPARATOR) ? strrchr(argv[0], PATH_SEPARATOR) + 1 : argv[0];
      fprintf(stderr, "Failure to open '%s'\n"
                      "Usage:\t%s\t\t\t- instruction tests\n"
                      "\t%s file\t\t\t- disassemble file\n"
                      "\t%s file offset size\t- disassemble file section\n",
              argv[1], pgmname, pgmname, pgmname);
    }
  } else {
    disasm_init(&arm, 0);

    testthumb(&arm, 0xe001, 0,      2, "b       0x0000006");
    testthumb(&arm, 0xd33a, 0,      2, "bcc     0x000007a");
    testthumb(&arm, 0xd048, 0,      2, "beq     0x0000098");
    disasm_address(&arm, 0x0800049c);
    testthumb(&arm, 0xe7ea, 0,      2, "b       0x8000474");
    disasm_address(&arm, 0x0800052e);
    testthumb(&arm, 0xf7ff, 0xfed1, 4, "bl      0x80002d4");
    disasm_address(&arm, 0x08000424);
    testthumb(&arm, 0xdbfa, 0,      2, "blt     0x800041c");
    testthumb(&arm, 0x4770, 0,      2, "bx      lr");
    testthumb(&arm, 0x4790, 0,      2, "blx     r2");
    disasm_address(&arm, 0x00a8);
    testthumb(&arm, 0xf7ff, 0xefea, 4, "blx     0x0000080");
    ...
    testthumb(&arm, 0xb12c, 0,      2, "cbz     r4, 0x8001598");
    ...
    testthumb(&arm, 0xdf01, 0,      2, "swi     #1");
    ...
    disasm_address(&arm, 0x00a8);
    testarm(&arm, 0x3afffffb, "bcc     0x000009c");
    testarm(&arm, 0xe12fff12, "bx      r2");
    testarm(&arm, 0xe12fff32, "blx     r2");
    disasm_address(&arm, 0x00a8);
    testarm(&arm, 0xfafffff4, "blx     0x0000080");
    testarm(&arm, 0x0000049c, "muleq   r0, ip, r4");
    testarm(&arm, 0x3fffcfff, "swicc   0xffcfff");
